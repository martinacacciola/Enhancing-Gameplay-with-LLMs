...
export async function mineBlock(bot, blockType, num=1) {
    /**
     * Mine the specified number of blocks of the given type.
     * @param {MinecraftBot} bot - Reference to the minecraft bot.
     * @param {string} blockType - The type of block to mine.
     * @param {number} num - The number of blocks to mine. Defaults to 1.
     * @returns {Promise<boolean>} - True if the blocks were mined, false otherwise.
     * @example
     * await skills.mineBlock(bot, "diamond_ore", 5);
     **/
    let blocksMined = 0;
    while (blocksMined < num) {
        let block = world.getNearestBlock(bot, blockType, 16);
        if (!block) {
            log(bot, `No ${blockType} blocks found.`);
            return false;
        }
        if (bot.entity.position.distanceTo(block.position) > 4.5) {
            await goToPosition(bot, block.position.x, block.position.y, block.position.z);
        }
        await breakBlockAt(bot, block.position.x, block.position.y, block.position.z);
        blocksMined++;
    }
    log(bot, `Mined ${num} ${blockType} blocks.`);
    return true;
}


export async function fillBinWithWater(bot) {
    /**
     * Fill an empty bin with water.
     * @param {MinecraftBot} bot - Reference to the minecraft bot.
     * @param {Vec3} binPosition - The position of the empty bin.
     * @returns {Promise<boolean>} - True if the bin was filled with water, false otherwise.
     * @example
     * let binPos = new Vec3(10, 64, 10);
     * await skills.fillBinWithWater(bot, binPos);
     **/
    const waterSource = world.getNearestBlock(bot, "water", 16);
    if (!waterSource) {
        log(bot, "No water source found.");
        return false;
    }
    
    if ( world.getInventoryCounts(bot)['bucket'] < 1) {
        log(bot, "You don't have bin.");
        return false;
    }
    
    if (bot.entity.position.distanceTo(waterSource.position) > 4.5) {
        await goToPosition(bot, waterSource.position.x, waterSource.position.y, waterSource.position.z);
    }
    
    await placeBlock(bot, 'bucket', waterSource.position.x, waterSource.position.y, waterSource.position.z);
    log(bot, `Filled the bin at with water.`);
    return true;
